...
export const contact = catchAsyncError(async (req, res, next) => {
  ...
  const subject = "Contact form from LearnXp";
  const text = `Hello , I am ${name} and my Email is ${email}. \n${message}`;
...
export const courseRequest = catchAsyncError(async (req, res, next) => {
  ...
  const to = process.env.MY_MAIL;
  const subject = "Course Request On LearnXp";
  const text = `Hello ,I am ${name} and my Email is ${email}. \n${course}`;
  ...
  res.status(200).json({
    success: true,
    message: "Your Course Request Has Been Submitted.",
  });
});
...
export const getDashboardStats = catchAsyncError(async (req, res, next) => {
 ...
 // last one year data
  ...
  // arr.unshift for adding in the start
